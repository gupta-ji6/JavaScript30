...
transform: scale(-1, 1); filter: FlipH;';
  ...
  return setInterval(function () {
    ctx.drawImage(video, 0, 0, width, height); //   take pixels out

    var pixels = ctx.getImageData(0, 0, width, height); // mess with them
    // pixels = redEffect(pixels);

    pixels = rgbSplit(pixels);
    ctx.globalAlpha = 0.8; // put them back

    ctx.putImageData(pixels, 0, 0); // debugger;
  }, 16);
}
...
function takePhoto() {
  ...
  link.innerHTML = "<img src=\"".concat(data, "\" alt='Smart Man' />");
  strip.insertBefore(link, strip.firstChild);
}

function redEffect(pixels) {
  for (var _i = 0; _i < pixels.data.length; _i += 4) {
    pixels.data[_i + 0] += 100; //r

    pixels.data[_i + 1] -= 50; //g

    pixels.data[_i + 2] *= 0.5; //b
  }
